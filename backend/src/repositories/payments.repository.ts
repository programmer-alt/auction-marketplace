...
export const getPaymentByStripeId = async (prisma: PrismaClient, stripePaymentId: string) => {
  return await runWithRetry(() => prisma.payment.findFirst({ where: { stripePaymentId } }));
};

// Обновление платежа
export const updatePayment = async (prisma: PrismaClient, id: number, data: UpdatePaymentData) => {
  return await runWithRetry(() => prisma.payment.update({ where: { id }, data }));
};
...
export const getPaymentsCountByUserId = async (prisma: PrismaClient, userId: number) => {
  return await runWithRetry(() => prisma.payment.count({ where: { userId } }));
};
...
export const updateAuctionPaidAt = async (prisma: PrismaClient, auctionId: number) => {
  return await runWithRetry(() => prisma.auction.update({ where: { id: auctionId }, data: { paidAt: new Date() } }));
};

// Удаление платежа
export const deletePayment = async (prisma: PrismaClient, id: number) => {
  return await runWithRetry(() => prisma.payment.delete({ where: { id } }));
};
